...
// ジェネレータ関数から呼び出すジェネレータ関数の用意
function* y(i){
    for(let ii=0; ii<10; ii++){
        console.log(`---#003---- i=${i-1},ii=${ii}`);
        yield    
    }
}
// ジェネレータ関数
...
// ジェネレータ関数
function* test2(){
    console.log('test2---#001');
    yield
    console.log('test2---#002');
    yield
    let i = 0
    while(i < 15) {
        console.log(`test2---#003 ${i}`);
        i+=1
        yield
    }
}
...
//clock.register(test1);
clock.register(test2);
