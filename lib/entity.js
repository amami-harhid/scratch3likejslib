...
//    }
     ...
     */
    async $isTouchingColor(value){
        const rgb = Libs.default.Cast.toRgbColorObject(value);
        let r= rgb.r | 0;
        let g= rgb.g | 0;
        let b= rgb.b | 0;
        if(this.render && this.render.renderer){
            const _renderer = this.render.renderer;
            return await _renderer.isTouchingColor(this.drawableID, [r,g,b]);
        }
        return false;
    }
    /**
    ...
    async $colorIsTouchingColor(targetRgb, maskRgb) {
        if(targetRgb && maskRgb ){
            if(Array.isArray(targetRgb) && Array.isArray(maskRgb)){
                if(this.render && this.render.renderer){
                    const _renderer = this.render.renderer;
                    return await _renderer.isTouchingColor(this.drawableID, targetRgb, maskRgb);
                }    
            }
        }
        return false;
