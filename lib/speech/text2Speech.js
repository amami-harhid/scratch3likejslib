...
const Speech = class {
    ...
    async speechAndWait(self, words, properties={}) {
        // 128文字までしか許容しないとする
        const text = encodeURIComponent(words.substring(0, 128));
        let path = `${SERVER_HOST}/synth?locale=${this.locale}&gender=${this.gender}&text=${text}`;
        if(!this.cache.has(path)) {
        ...
        await this._speechPlayUntilDone(self, sound.name, sound.data, properties);
    }
    
    async _speechPlayUntilDone(self, name, data, properties) {
        const sounds = new Sounds();
        await sounds.setSound(name, data, properties);
        await sounds.startSoundUntilDone(self);
    }
